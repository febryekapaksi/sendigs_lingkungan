            ...
            <table class="table custom-table">
                ...
                <tbody>
                    ...
                    foreach ($list_kasbon_akomodasi as $item) {
                        ...
                        echo $no;
                        echo '<input type="hidden" name="req_others[' . $no . '][id_detail]" value="' . $item->id . '">';
                        echo '<input type="hidden" name="req_others[' . $no . '][id_item]" value="' . $item->id_item . '">';
                        echo '<input type="hidden" name="req_others[' . $no . '][nm_item]" value="' . $item->nm_item . '">';
                        echo '</td>';
                        echo '<td class="text-left">' . $item->nm_biaya . '</td>';

                        echo '<td class="text-center">';
                        echo number_format($item->qty_final);
                        echo '<input type="hidden" name="req_others[' . $no . '][qty_estimasi]" value="' . $item->qty_final . '">';
                        echo '</td>';

                        echo '<td class="text-center">';
                        echo number_format($item->price_unit_final, 2);
                        echo '<input type="hidden" name="req_others[' . $no . '][price_unit_estimasi]" value="' . $item->price_unit_final . '">';
                        echo '</td>';

                        echo '<td class="text-center">';
                        echo number_format($item->total_final, 2);
                        echo '<input type="hidden" class="form-control form-control-sm" name="req_others[' . $no . '][total_budget]" value="' . $item->total_final . '">';
                        echo '</td>';

                        echo '<td>';
                        echo '<input type="number" class="form-control form-control-sm text-right" name="req_others[' . $no . '][qty_budget_tambahan]" min="0" value="0" onchange="hitung_all()">';
                        echo '</td>';
                        ...
                        echo '<td>';
                        echo '<input type="text" class="form-control form-control-sm text-right auto_num" name="req_others[' . $no . '][pengajuan_new_budget]" value="' . $item->total_final . '" readonly>';
                        echo '</td>';

                        echo '<td>';
                        echo '<input type="text" class="form-control form-control-sm" name="req_others[' . $no . '][reason]">';
                        echo '</td>';
...
<script>
    ...
    function hitung_all() {
        ...
        for (i = 1; i <= no; i++) {
            ...
            var budget_tambahan = get_num($('input[name="req_others[' + i + '][budget_tambahan]"]').val());
    ...
    $(document).on('submit', '#frm-data', function(e) {
        ...
        swal({
            ...
            if (next) {
                ...
                $.ajax({
                    type: 'post',
                    url: siteurl + active_controller + 'save_request_budget_others',
                    data: formData,
                    cache: false,
                    processData: false,
                    contentType: false,
                    ...
                    success: function(result) {
                        if (result.status == '1') {
                            swal({
                                ...
                                window.location.href = siteurl + active_controller + "add_kasbon_others/<?= urlencode(str_replace('/', '|', $list_budgeting->id_spk_budgeting)) ?>";
                            });
                        } else {
                            swal({
                                type: 'warning',
